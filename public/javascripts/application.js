...
var _scms = new Date().getMilliseconds(), _tnow = _scms;
var _test, _ajax, _shot = "", _sclr = "";
...
function setDisp(idx, pos, pls) {
  ...
  if (/^( *[0-9]+\:)?`RS([0-9]+)/.exec(data)) {
    ...
    if (pos) {
      ...
      var w = getWidth(data), wb = _size[idx] - w;
      if (_scrw[idx] != wb) {
...
function changeDisp() {
  var f = 0;
  for (var idx = 0; idx < DISP_SIZE; idx++) for (var pos = 0; pos < 2; pos++) {
    if (!_time[idx][pos] || --_time[idx][pos]) continue;
    nextDisp(idx, pos, 0);
    f = 1;
  }
  if (f) {
    for (var idx = 0; idx < DISP_SIZE; idx++) resetSync(idx);
    for (var idx = 0; idx < DISP_SIZE; idx++) resetDisp(idx);
    for (var idx = 0; idx < DISP_SIZE; idx++) resetTime(idx);
  }
...
function scrollDisp() {
  var f = 0;
  for (var idx = 0; idx < DISP_SIZE; idx++) {
...
function resetData(text) {
  ...
  for (var idx = 0; idx < DISP_SIZE; idx++) data[idx] = [[], []];
  for (var leng = lines.length, i = 0; i < leng; i++) {
    var line = lines[i];
    if (!/^ *([0-9]+)([A-Ca-c]?)\:(.+)$/.exec(line)) continue;
    var idx = RegExp.$1 - 0, pos = RegExp.$2.length ? "abcABC".indexOf(RegExp.$2) : -1;
...
window.onload = function() {
  ...
  var data = [
    ...
    // より本物に近いサンプル
    // "12a:0:`SW",
    // "12a:100:@o10:30 普通　東　　京　　",
    // "12b:100:@g８両編成",
    // "12a:70:@o10:30 LOCAL Tokyo     　",
    // "12b:70:@g８Cars",
    // "13a:0:`WD12",
    // "13a:100:@o10:37 @r特急@o　東　　京　　",
    // "13b:100:@g８両編成",
    // "13a:70:@o10:37 @rRAPID@o Tokyo     　",
    // "13b:70:@g８Cars",
    // "13a:100:@o10:37 @r特急@o　東　　京　　",
    // "13b:100:@g８両編成",
    // "13a:70:@o10:37 @rRAPID@o Tokyo     　",
    ...
    // "13a:70:@o10:37 @rRAPID@o Tokyo     　",
    // "13b:70:@g８Cars",
    // "13B:0:　　@g【@rダイヤ乱れ@g：@rサンプル線@g　8時23分頃、@rサンプル駅付近@gで@r車両トラブル@gのため@g】@g【@r遅延@g：@rサンプル線@g　@r混雑@gのため@g】　　",
    // "14a:100:@G　快　速　@w 10 : 30 新宿",
    // "14b:100:@g８両編成",
    // "14a:70:@GＲＡＰＩＤ@w 10 : 30 SHINJUKU",
    // "14b:70:@g８Cars",
    // "15a:0:@g停車駅@g　",
    // "15b:0:@w赤羽・十条・板橋・池袋・新宿・渋谷・恵比寿・大崎"
  ];
  resetData(data.join("\r\n"));
}
